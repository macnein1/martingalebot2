...
def ensure_ring_handler(logger_name: str = "mlab", level: int = logging.INFO, 
    ...
    with _registry_lock:
        ...
        for existing_handler in logger.handlers:
            if isinstance(existing_handler, RingBufferLogHandler) and existing_handler.name == f"{logger_name}.ring":
...
def tail_logs(logger_name: str = "mlab", last_n: int = 200, as_json: bool = False) -> Iterable:
    """Get the most recent logs from the ring buffer. Returns list for JSON, generator for text."""
    handler = ensure_ring_handler(logger_name, use_json=as_json)
    
    if as_json:
        return handler.tail_json(last_n)
    else:
        return _tail_logs_generator(handler, last_n)


def _tail_logs_generator(handler: RingBufferLogHandler, last_n: int):
    """Generator function for text logs to avoid making tail_logs a generator function"""
